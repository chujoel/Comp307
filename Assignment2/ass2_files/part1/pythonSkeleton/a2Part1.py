...
if __name__ == '__main__':
    ...
    initial_hidden_bias = np.array([0, 0])
    initial_output_bias = np.array([0, 0, 0])
    ...
    print('Weights before performing BP for first instance only:')
    print('Hidden layer weights:\n', nn.hidden_layer_weights)
    print('Output layer weights:\n', nn.output_layer_weights)

    # nn.train([instances[0]], [onehot_encoded[0]], 1, integer_encoded)
    ...
    for i in range(len(predictions)):
        if str(predictions[i]) in str(integer_encoded[i]):
            accuracy+=1
    print('acc = ', accuracy)
    print("Accuracy is",accuracy,"/",len(test_instances)," which is ",accuracy/len(test_instances),"%")
    predictionList = nn.getPredictionList()
